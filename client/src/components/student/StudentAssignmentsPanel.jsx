          ...
          return (
            <div key={a._id} className={`p-5 rounded-2xl border shadow-sm transition group ${locked ? 'bg-gray-50 opacity-80' : 'bg-white hover:shadow-md'}`}>
                ...
                <div className="text-right text-xs">
                  {a.nextGameAttemptsRemaining === 0 && a.nextGameId && (
                    <>
                      ...
                      <div className="text-xs text-gray-500">{t.allAttemptsUsed}</div>
                    </>
                  )}
                  {a.nextGameAttemptsRemaining > 0 && a.nextGameId && (
                    <>
              ...
              <div className="mt-4 flex gap-2 flex-wrap">
                ...
                >
                  {a.nextGameAttemptsRemaining > 0 && a.nextGameId ? (t.continue || 'Continue') : (t.completed || 'Completed')}
                </Link>
                <button onClick={() => openBreakdown(a._id)} className="text-xs px-3 py-1.5 rounded-md font-medium border bg-white hover:bg-indigo-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-300">{t.details || 'Details'}</button>
