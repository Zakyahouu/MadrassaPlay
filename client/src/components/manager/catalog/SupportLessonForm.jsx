...
const SupportLessonForm = ({ isOpen, onClose, onSubmit, data, catalog }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      alert(t.failedToSaveLesson || 'Failed to save lesson');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-semibold text-gray-900">
              {data ? (t.editSupportLesson || 'Edit Support Lesson') : (t.addSupportLesson || 'Add Support Lesson')}
            </h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex items-center justify-between p-6 border-t border-gray-200">
          ...
          >
            {loading ? (
              ...
              <>
                {(data ? (t.update || 'Update') : (t.create || 'Create'))} {formData.selectedGrades.length > 1 ? (t.lessons || 'Lessons') : (t.lesson || 'Lesson')}
