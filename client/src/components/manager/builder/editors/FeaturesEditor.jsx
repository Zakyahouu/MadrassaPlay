...
const FeaturesEditor = ({ data, onChange, showMessage }) => {
  ...
  const addFeature = () => {
    ...
    showMessage('Feature added', 'success');
  };

  const removeFeature = (index) => {
    const updatedFeatures = features.filter((_, i) => i !== index);
    onChange({ ...data, features: updatedFeatures });
    showMessage('Feature removed', 'success');
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600">Highlight your platform's key features</p>
        </div>
        <button
          onClick={addFeature}
      ...
      {features.length === 0 && (
        <div className="text-center py-12 bg-gray-50 rounded-lg border-2 border-dashed">
          <p className="text-gray-500">No features yet. Click "Add Feature" to get started.</p>
        </div>
      )}
    </div>
  );
