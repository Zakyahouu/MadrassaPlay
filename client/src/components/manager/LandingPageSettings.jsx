...
const LandingPageSettings = () => {
  ...
  const handlePreview = () => {
    ...
    if (!schoolId) {
      alert('No school assigned to preview.');
      return;
    }
    ...
    <div className="space-y-4 bg-white p-6 rounded-lg border border-gray-200">
      ...
      <div>
        <label className="inline-flex items-center gap-2">
          <input type="checkbox" checked={data.isEnabled} onChange={(e) => setData({ ...data, isEnabled: e.target.checked })} /> Enable public page
        </label>
      </div>
      <div>
        <label className="block text-sm text-gray-600">Hero Title</label>
        <input value={data.heroTitle} onChange={(e) => setData({ ...data, heroTitle: e.target.value })} className="w-full p-2 border rounded"/>
      </div>
      <div>
        <label className="block text-sm text-gray-600">About Section</label>
      ...
      <div>
        ...
        <div className="flex gap-2 items-center">
          ...
          <small className="text-xs text-gray-500">Upload an image (JPG, PNG)</small>
        </div>
      ...
      <div className="flex gap-2">
        <button className="px-4 py-2 bg-blue-600 text-white rounded" disabled={saving} onClick={onSave}>{saving ? 'Saving...' : 'Save'}</button>
        <button
          className="px-4 py-2 bg-white border text-gray-700 rounded hover:bg-gray-50"
          onClick={handlePreview}
          disabled={!user?.school}
          title={user?.school ? 'Open public school landing page in a new tab' : 'Assign a school to your account to preview'}
        >
          Preview
        </button>
      </div>
